...
import { caseStages, type CaseStage, type LiveComplaint } from "@/src/lib/mock/types";
import type { BankFixture } from "@/src/lib/mock/types";
      ...
      <section className="panel" aria-labelledby="timeline-heading">
        ...
        <ol className="m-0 grid list-none gap-0 p-0">
          {caseStages.map((item, index) => {
            ...
            return <li className="grid grid-cols-[2rem_1fr] gap-3" key={item}><div className="flex flex-col items-center"><span className={`grid size-7 place-items-center border-2 ${complete ? "border-[#08745c] bg-[#08745c] text-white" : current ? "border-[#e45118] bg-[#fff4df] text-[#a83308]" : "border-[#8292a2] bg-white text-[#8292a2]"}`}>{complete ? <Check aria-hidden="true" size={17} /> : <Circle aria-hidden="true" size={12} fill={current ? "currentColor" : "none"} />}</span>{index < caseStages.length - 1 ? <span className={`min-h-10 w-0.5 flex-1 ${complete ? "bg-[#08745c]" : "bg-[#cbd5df]"}`} /> : null}</div><div className="pb-5"><strong className={current ? "text-[#a83308]" : ""}>{stageMeta[item].label}</strong><p className="m-0 mt-1 text-sm text-[#52606d]">{stageMeta[item].citizenCopy}</p>{item === "hold_placed" && index <= currentIndex ? <p className="mb-0 mt-2 inline-flex items-center gap-2 bg-[#eaf6f2] px-2 py-1 text-sm font-black text-[#075a49]"><Landmark aria-hidden="true" size={16} /> ₹{complaint.holdAmount.toLocaleString("en-IN")} at {bank?.name ?? "beneficiary bank"}</p> : null}</div></li>;
          })}
        </ol>
      </section>
