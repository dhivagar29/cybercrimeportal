      ...
      <div className="mt-6 grid gap-3">
        {complaints.map((complaint) => (
          <article className="panel grid gap-3 md:grid-cols-[1fr_auto] md:items-center" key={complaint.id}>
            <div><div className="flex flex-wrap items-center gap-2"><h2 className="m-0 text-lg">{complaint.id}</h2><span className="status-pill text-[#08745c]">{stageLabel[complaint.stage]}</span></div><p className="mb-0 text-sm text-[#52606d]">{complaint.subcategory} · {complaint.district}, {complaint.state} · ₹{complaint.amount.toLocaleString('en-IN')}</p></div>
